...
func New(certConf *CertConfig) (*LegoCMD, error) {
	...
	defaultPath = filepath.Join(p, "certs", sanitizedDomain(certConf.CertDomain))
	lego := &LegoCMD{
		Conf: certConf,
		path: defaultPath,
	}
...
func (l *LegoCMD) DNSCert() (CertPath string, KeyPath string, err error) {
	...
	for key, value := range l.Conf.DNSEnv {
		os.Setenv(strings.ToUpper(key), value)
	}

	// First check if the certificate exists
	CertPath, KeyPath, err = checkCertFile(l.Conf.CertDomain)
	if err == nil {
...
func checkCertFile(domain string) (string, string, error) {
	keyPath := path.Join(defaultPath, baseCertificatesFolderName, fmt.Sprintf("%s.key", domain))
	certPath := path.Join(defaultPath, baseCertificatesFolderName, fmt.Sprintf("%s.crt", domain))
